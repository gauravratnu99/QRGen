...
export const QrPreviewCard: React.FC<QrPreviewCardProps> = ({ config }) => {
  ...
  return (
    <div
      ...
      className="relative flex flex-col items-center justify-between p-6 sm:p-8 rounded-2xl bg-slate-900/60 backdrop-blur-md border border-slate-800 shadow-lg overflow-hidden transition-all duration-300"
    >
      {/* Top action header */}
      <div className="w-full flex items-center justify-between mb-4 z-10">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-white/5 text-slate-400 hover:text-slate-200 border border-white/10 hover:bg-white/10 transition-colors"
      ...
      <div
        id="qr-canvas-dock"
        className={`relative flex items-center justify-center p-4 sm:p-6 rounded-2xl transition-all duration-300 z-10 shadow-2xl ${
          config.transparentBg
            ? 'bg-[linear-gradient(45deg,#1e293b_25%,transparent_25%),linear-gradient(-45deg,#1e293b_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#1e293b_75%),linear-gradient(-45deg,transparent_75%,#1e293b_75%)] bg-[size:16px_16px] bg-[position:0_0,0_8px,8px_-8px,-8px_0]'
            : ''
        }`}
        style={{
      ...
      <div className="w-full mt-4 text-center z-10 space-y-2">
        <div className="flex items-center justify-center gap-1.5 text-xs text-slate-400 truncate px-2">
      ...
      <div className="w-full grid grid-cols-2 gap-2 mt-4 z-10">
        <button
          ...
          className="flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 border border-white/10 text-xs font-medium text-slate-200 transition-all hover:border-cyan-500/30 active:scale-[0.98]"
        >
          {copyImageSuccess ? (
            <>
        ...
        >
          {copySvgSuccess ? (
            <>
              <Check className="w-3.5 h-3.5 text-emerald-400" />
              <span className="text-emerald-300">Copied SVG</span>
            </>
          ) : (
            <>
      ...
      {showScanSimulator && (
        <div
          ...
          className="w-full mt-4 p-3.5 rounded-2xl bg-cyan-950/40 border border-cyan-500/30 text-left z-10 animate-in fade-in slide-in-from-top-2 duration-200"
        >
          ...
          <div className="p-2.5 rounded-xl bg-slate-950/70 border border-cyan-500/20 mb-2 font-mono text-xs text-cyan-200 break-all">
            {displayUrl}
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1 text-xs font-semibold text-cyan-300 hover:text-cyan-200 bg-cyan-500/20 hover:bg-cyan-500/30 px-2.5 py-1 rounded-lg transition-colors"
            >
